...
void Subgraph::reset(const std::vector<Graph::EdgeId>& new_original_edgeids) {
    ...
    _this_graph.clear_graph();
    unsigned int new_num_nodes = 0;

    _subgraph_nodeids_of_nodes_in_originalgraph.assign(_original_graph.num_nodes(), Graph::invalid_node_id);

    _original_nodeids.clear();

    //überschreibe die alten original_edge_ids
    _original_edgeids = new_original_edgeids;

    //füge alle zu den neuen Kanten inzidenten Knoten hinzu

    for( auto curr_or_edge_id: _original_edgeids) {
        ...
        const Graph::Edge& curr_or_edge = _original_graph.get_edge(curr_or_edge_id);
        const std::vector<Graph::NodeId> nodes_to_add = {curr_or_edge.node_a(), curr_or_edge.node_b()};

        for( auto node_to_add_id: nodes_to_add) {

            //prüfe, ob Knoten bereits hinzugefügt
            if (_subgraph_nodeids_of_nodes_in_originalgraph[node_to_add_id] == Graph::invalid_node_id) {

                //füge Knoten zu Graph hinzu
                const Graph::Node& node_to_add = _original_graph.get_node(node_to_add_id);
                _this_graph.add_one_node(node_to_add.terminal_state());

                //aktualisiere new_sub_nodeids_of_or_nodes
                _subgraph_nodeids_of_nodes_in_originalgraph[node_to_add_id] = new_num_nodes;
                new_num_nodes++;

                //aktualisiere new_or_node_ids
                _original_nodeids.push_back(node_to_add_id);
            }
        }

        //füge die entsprechende Kante hinzu (siehe Initialisierung von nodes_to_add)
        _this_graph.add_edge( _subgraph_nodeids_of_nodes_in_originalgraph[nodes_to_add[0]],
                              _subgraph_nodeids_of_nodes_in_originalgraph[nodes_to_add[1]],
                              curr_or_edge.weight());
    }
