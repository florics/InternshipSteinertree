...
namespace SteinerVertexElim {
    ...
    // gibt die Nachbarschaft der Eingabe-Lösung aus (als Veränderungen ausgehend von der Eingabe-Lösung, siehe auch Ausgabe von process_node)
    // falls ein move per pass eingegeben wird, so wird die gesamte Nachbarschaft ausgegeben
    // falls mehrere moves per pass eingegeben wird, so wird eine Reihe von Verbesserungen ausgegeben, die simultan ausgeführt werden können
